...
export default function Last24H() {
  const [randomA, setRandomA] = useLocalStorage('last24H', []);
  ...
  // Change the colors after 24h has passed
  useEffect(() => {
    const startKey = 'last24H_start';
    ...
    // If no start time found, set it now
    if (!startTime) {
      startTime = now;
    ...
    const remaining = 86400000 - elapsed; // 24h in ms

    // If 24h already passed, reset immediately
    if (remaining <= 0) {
      localStorage.removeItem('last24H');
      localStorage.removeItem(startKey);
      setRandomA([]); // Trigger regeneration
      return;
    }
  ...
  return (
    <div className="px-4 pb-4">
      <h1 className="text-2xl w-full py-2">Recently Generated </h1>
      <hr />
